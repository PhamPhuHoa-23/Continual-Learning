...
class Wrapper(nn.Module):
    def __init__(self, bb, ps): super().__init__(
    ); self.backbone = bb; self.prim_sel = ps

    def forward(self, imgs, **kw):
        out = self.backbone(imgs, **kw)
        r = {"recon": out["reconstruction"],
             "mask": out["hard_keep_decision"], "slots": out["slots"], **out}
...
trainer = AdaSlotTrainer(
    config=cfg, slot_model=slot_model, primitive_predictor=None)
