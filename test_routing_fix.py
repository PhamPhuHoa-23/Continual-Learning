...
assert assignments.shape == (
    B, K), f"Expected ({B},{K}), got {assignments.shape}"
assert assignments.dtype == torch.long
assert assignments.min() >= 0 and assignments.max() < N_AGENTS
print(
    f"  assignments.shape={tuple(assignments.shape)}  range=[{assignments.min()},{assignments.max()}]  PASS")
...
assert weights.shape == (
    B, K, N_AGENTS), f"Expected ({B},{K},{N_AGENTS}), got {weights.shape}"
assert abs(weights.sum(dim=-1).mean().item() -
           1.0) < 1e-4, "weights should sum to 1 over agents"
print(
    f"  weights.shape={tuple(weights.shape)}  sum_over_agents≈{weights.sum(dim=-1).mean():.4f}  PASS")
...
assert assignments_slda.shape == (
    B, K), f"Expected ({B},{K}), got {assignments_slda.shape}"
